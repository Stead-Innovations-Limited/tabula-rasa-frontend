...
export default async function getParticularEventRegistration(eventId: string) {
  try {
      ...
      if (
        ...
        throw new Error("Token is required to fetch your event registration.");
      }
      ...
      const response = await tryCatch(async () => {
        return await axios.get(
          `https://tabula-rasa-backend.up.railway.app/events/id/${eventId}/participants`,
          {
            headers: {
      ...
      return {
        error: true,
        errorData: error,
        message: "Failed to fetch your event registration.",
      };
    }
}